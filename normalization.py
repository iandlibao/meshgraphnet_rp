...
class Normalizer(nn.Module):
    ...
    def forward(self, batched_data, node_num=None, accumulate=True):
        ...
        return (batched_data.to(device) - self._mean()) / self._std_with_epsilon()
    ...
    def _accumulate(self, batched_data, node_num=None):
        ...
        data_sum = torch.sum(batched_data, dim=0).to(device)
        squared_data_sum = torch.sum(batched_data ** 2, dim=0).to(device)
